...
vibe() {
    ...
    _connect_to_remote() {
        ...
        if [ "$use_cly" = "true" ]; then
            ...
            ssh -i "$SSH_KEY_PATH" "$SSH_USER_HOST" -t "cd '$REMOTE_WORKTREE_BASE/$REPO_NAME/$WORKTREE_NAME' && security -v unlock-keychain -p admin ~/Library/Keychains/login.keychain-db && export TMPDIR=\$(mktemp -d) && zsh -l -i -c \"$CODING_TOOL_CMD\""
        else
            echo "Connecting to vibecoding.local and navigating to worktree..."
            ssh -i "$SSH_KEY_PATH" "$SSH_USER_HOST" -t "cd '$REMOTE_WORKTREE_BASE/$REPO_NAME/$WORKTREE_NAME' && security -v unlock-keychain -p admin ~/Library/Keychains/login.keychain-db && export TMPDIR=\$(mktemp -d) && zsh -l -i"
        fi
    }
