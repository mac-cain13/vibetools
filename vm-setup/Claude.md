...
- **`sentry`** — Sentry's first-party CLI (installed via Homebrew) for investigating production errors, crashes, and user-reported issues: search issues, read events/stack traces, query traces/logs/replays, and pull Seer AI analysis (`sentry issue explain`/`plan`). Reach for it whenever a bug or "a user is seeing X" comes up and the project reports to Sentry. See the `sentry` skill, or discover commands with `sentry --help` / `sentry <group> --help`.
- **`linear`** — Linear's CLI (installed via Homebrew as `schpet/tap/linear`) for managing Linear issues, projects, cycles, and comments from the terminal, with git branch-name integration. Reach for it whenever a Linear issue/ticket (e.g. `ENG-123`) comes up or the user wants to read/create/update/comment on Linear work. See the `linear-cli` skill, or discover commands with `linear --help` / `linear <group> --help`.
